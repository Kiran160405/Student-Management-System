...
def delete_student():
    ...
    if selected:
        ...
        message_label.config(text="Student data deleted successfully", fg="green")
    else:
        message_label.config(text="Select a student data to delete", fg="red")
...
def add_student():
    ...
    if name == "" or age == "" or course == "Select Course" or gender == "":
        message_label.config(text="Please enter all details", fg="red")
        return

    if not age.isdigit():
        message_label.config(text="Age must be  number", fg="red")
        return
...
title_label = tk.Label(root, text="Student Management System",font=("Arial", 16, "bold"))
